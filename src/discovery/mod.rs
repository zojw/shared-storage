...
pub use local::LocalSvcDiscovery;
...
pub mod local {
    ...
    use super::{Discovery, ServiceType, Svc};
    use crate::{
    ...
    struct inner {
        ...
        manifest_locator: Vec<(
            u32,
            LocatorServer<CacheServerLocator<MemBlobMetaStore<MemBlobStore>>>,
        )>,
        manifest_bucket_svc: Vec<(
            u32,
            ManifestBucketServiceServer<
                ManifestBucketService<MemBlobStore, MemBlobMetaStore<MemBlobStore>>,
            >,
        )>,
    }
    ...
    impl LocalSvcDiscovery {
        ...
        pub async fn register_manifest_svc(
            ...
            bucket_svc: ManifestBucketServiceServer<
                ManifestBucketService<MemBlobStore, MemBlobMetaStore<MemBlobStore>>,
            >,
            locator: LocatorServer<CacheServerLocator<MemBlobMetaStore<MemBlobStore>>>,
        ) {
            let mut inner = self.inner.lock().await;
    ...
    #[async_trait]
    impl Discovery for LocalSvcDiscovery {
        async fn list(&self, svc_type: ServiceType) -> Result<Vec<Svc>> {
